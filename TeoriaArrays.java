    ...
    */
     ...
     */
            private static int detectarCuantasRasSuspendidas(int[] notasModulo485)
        {
            ...
            for (int indice = 0; indice < notasModulo485.length; indice++) {
                if (notasModulo485[indice]<5) {
                    //si entro está suspendida
                    contRaSuspendidas++;
                }
            } return contRaSuspendidas;
        } 

        private static int detectarCuantosSobresalientes(int[] notasModulo485)
        {
            int contRaSobresalientes=0;
            for (int indice = 0; indice < notasModulo485.length; indice++) {
                if (notasModulo485[indice]>=9) {
                    //si entro está suspendida
                    contRaSobresalientes++;
                }
            } return contRaSobresalientes;

        } 

        private static boolean recuperacionesJunio(int[] notasModulo485)
        {
            int ra = 0; //indice
            ...
            } while(ra<notasModulo485.length && suspendido == false);
        ...
        private static int consultarNotaRa (int[] notasModulo485){
            Scanner sc = new Scanner (System.in);
            System.out.println("¿Qué nota quieres saber?");
            int saber_nota = sc.nextInt();
            return notasModulo485[saber_nota - 1];
        }
